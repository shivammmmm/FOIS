...
export default function InwardMonitor() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await base44.movements.page({ direction: "Inward", page, limit: PER_PAGE, search: filters.search, division: filters.division === "All" ? [] : [filters.division], state: filters.states, district: filters.districts, station: filters.stations, commodity: filters.commodities, rake: filters.rakeCmdts });
