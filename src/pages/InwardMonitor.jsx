...
export default function InwardMonitor() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const inwardOnly = (data || []).filter((record) => record.movement_type === "Inward");
  ...
  const options = useMemo(() => {
    ...
    scopedByDivision.forEach((record) => {
      ...
      if (record.station_to) {
        stations.set(record.station_to, formatStationNameAndCode(record.station_to));
      }
    });
