  ...
  },
  {
    ...
    fields: [
      { name: "code", label: "Commodity Code", placeholder: "POL" },
      {
        ...
        placeholder: "Petroleum Oil & Lubricants",
      },
    ],
  ...
  },
  {
    ...
    fields: [
      { name: "code", label: "Company Code", placeholder: "IOCL" },
      {
        name: "name",
        label: "Company Name",
        placeholder: "Indian Oil Corporation Limited",
      },
    ],
...
export default function MasterManagement() {
  ...
  const activeMaster = useMemo(
    () =>
      MASTER_TYPES.find((master) => master.key === activeKey) ||
      MASTER_TYPES[0],
    [activeKey]
  );
  ...
  async function loadRows() {
    ...
    try {
      ...
      setMessage({
        kind: "error",
        text: error?.message || "Failed to load master records.",
      });
    } finally {
      setLoading(false);
    }
  ...
  async function loadReferences() {
    try {
      ...
      setMessage({
        kind: "error",
        text: error?.message || "Failed to load dropdown masters.",
      });
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (editingId) {
        await apiClient.masterCatalog.update(
          activeKey,
          editingId,
          normalizePayload(form, activeMaster)
        );
        setMessage({ kind: "success", text: `${activeMaster.label} updated.` });
      } else {
        await apiClient.masterCatalog.create(
          activeKey,
          normalizePayload(form, activeMaster)
        );
      ...
      setMessage({
        kind: "error",
        text: error?.message || "Failed to save master record.",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function deleteRow(row) {
    if (!window.confirm(`Delete ${row.code || row.station_code || row.name}?`))
      return;
    setMessage({ kind: "info", text: "" });
    try {
      ...
      setMessage({
        kind: "error",
        text: error?.message || "Failed to delete master record.",
      });
    }
  }
  ...
  async function deleteAllDistricts() {
    ...
    try {
      ...
      setMessage({
        kind: "success",
        text: `${result?.deleted_count || 0} district record(s) deleted.`,
      });
    } catch (error) {
      setMessage({
        kind: "error",
        text: error?.message || "Failed to delete all district records.",
      });
    } finally {
      setSaving(false);
    }
    ...
    try {
      ...
      if (activeKey === "district") {
        const payloads = records.map((record) =>
          normalizePayload(
            buildPayloadFromImport(record, activeMaster),
            activeMaster
          )
        );
        const invalidIndex = payloads.findIndex((payload) =>
          activeMaster.validate(payload)
        );
        if (invalidIndex >= 0) {
          throw new Error(
            `Row ${invalidIndex + 2}: StateCode and DistrictName are required.`
          );
        }
        const result = await apiClient.masterCatalog.bulkImportDistricts(
          payloads
        );
        await Promise.all([loadRows(), loadReferences()]);
        setMessage({
          kind: result?.failed > 0 ? "warning" : "success",
          text: `Import finished. Imported/updated: ${
            result?.imported || 0
          }, Failed: ${result?.failed || 0}.${
            result?.fallback
              ? " Compatibility mode was used; deploy/restart the updated API for faster bulk imports."
              : ""
          }`,
        });
        return;
      ...
      for (const [recordIndex, record] of records.entries()) {
        ...
        try {
          await apiClient.masterCatalog.create(
            activeKey,
            normalizePayload(payload, activeMaster)
          );
          imported += 1;
        } catch (error) {
          failed += 1;
  ...
  async function exportRows() {
    ...
    XLSX.writeFile(
      workbook,
      `${activeMaster.label.replace(/\s+/g, "_")}_${Date.now()}.xlsx`
    );
  }
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage state, district, station, zone, division, commodity, company
            and product masters.
          </p>
        </div>
      </div>
      ...
      <main className="min-w-0 space-y-5">
        <section className="rounded-lg border border-border bg-card p-4 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Create, search, edit and delete {activeMasterName.toLowerCase()}{" "}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {activeMaster.importHeaders.map((header) => (
              <span
                key={header}
                className="rounded border border-primary/20 bg-primary/10 px-2 py-1 font-mono text-xs text-primary"
              >
                {header}
              </span>
            ))}
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="font-semibold text-foreground">
                {editingId
                  ? `Edit ${activeMasterName}`
                  : `Create ${activeMasterName}`}
              </h3>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {activeMaster.fields.map((field) => (
              <MasterField
                ...
                onChange={(value) =>
                  setForm((prev) =>
                    updateDependentForm(prev, activeKey, field.name, value)
                  )
                }
              />
            ))}
        ...
        <section className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
            <div>
              <h3 className="font-semibold text-foreground">
                Existing {activeMasterName} Records
              </h3>
                ...
                {loading ? (
                  [...Array(5)].map((_, rowIndex) => (
                    <tr key={rowIndex} className="border-b border-border/50">
                      {[...Array(activeMaster.columns.length + 1)].map(
                        (__, colIndex) => (
                  ...
                  <tr>
                    <td
                      colSpan={activeMaster.columns.length + 1}
                  ...
                  rows.map((row) => (
                    <tr
                      key={row.id}
                      className="border-b border-border/50 bg-white hover:bg-blue-50/60"
                    >
                      {activeMaster.columns.map((column) => (
