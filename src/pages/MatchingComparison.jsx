...
export default function MatchingComparison() {
  ...
  return <div className="space-y-6 p-4 md:p-6">
    ...
    <div className="overflow-x-auto rounded-xl border bg-card"><table className="w-full min-w-[2100px] text-sm"><thead className="border-b bg-muted/40 text-left text-xs uppercase text-muted-foreground"><tr>{["ODR Number","Division","Station From","Destination","CNSR / Company","CNSG","Commodity","Rake CMDT","Demand Date","Indented","Supplied","Balance","Matured Indent","Status","Confidence","Method","ODR Batch","Matured Batch","Matched On","Actions"].map((h)=><th key={h} className="px-3 py-3">{h}</th>)}</tr></thead>
      <tbody>{result.items.map((row)=><tr key={row.match_id} className="border-b border-border/60"><td className="px-3 py-3 font-medium">{display(row.odr?.odr_number)}</td><td className="px-3">{display(row.odr?.division)}</td><td className="px-3">{display(row.odr?.station_from)}</td><td className="px-3">{display(row.odr?.station_to)}</td><td className="px-3">{display(row.odr?.company || row.odr?.company_code)}</td><td className="px-3">{display(row.odr?.raw_data?.cnsg)}</td><td className="px-3">{display(row.odr?.commodity)}</td><td className="px-3">{display(row.odr?.rake_cmdt || row.odr?.rake_commodity_code)}</td><td className="px-3">{display(row.odr?.departure_date)}</td><td className="px-3">{row.indented_units}</td><td className="px-3">{row.supplied_units}</td><td className="px-3">{row.balance_units}</td><td className="px-3">{display(row.matured?.indent_number)}</td><td className="px-3"><span className="rounded-full bg-muted px-2 py-1 text-xs">{row.status}</span></td><td className="px-3">{Number(row.confidence)}%</td><td className="px-3">{row.match_method}</td><td className="px-3">{display(row.batch_odr)}</td><td className="px-3">{display(row.batch_matured)}</td><td className="px-3">{row.matched_on ? new Date(row.matched_on).toLocaleString() : "-"}</td><td className="px-3"><button onClick={()=>openDetail(row.match_id)} className="text-primary">View / Resolve</button></td></tr>)}
